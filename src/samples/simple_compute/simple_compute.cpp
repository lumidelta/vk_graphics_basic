...
void SimpleCompute::Execute()
{
  ...
  auto gpu = std::chrono::high_resolution_clock::now();
  ...
  ttl /= m_length;
  std::cout << "GPU-Result: " << ttl << std::endl;
  ...
  std::cout << "CPU-Time: "
            << std::chrono::duration<float, std::milli>{ std::chrono::high_resolution_clock::now() - cpu }.count()
            << std::endl;
  ttl = 0;
  for (int i = 0; i < source.size(); i++)
  {
  ...
  ttl /= m_length;
  std::cout << "CPU-Result: " << ttl << std::endl;
}
